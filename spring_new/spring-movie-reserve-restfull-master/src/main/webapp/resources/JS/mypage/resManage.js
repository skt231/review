...
function loadPage(page){
    $.ajax({
        //   /resManage/{page}/{pageRows}
        url : "../resManageajax/" + page + "/" + window.pageRows + "/" + res_memberUid,
        type : "GET",
        cache : false,
...
function updateList(jsonObj){
    ...
    if(jsonObj.status == "OK"){
        ...
        var chknow = year+"-"+month+"-"+date_now;
        ...
        for(i = 0; i < count; i++){
            ...
            result += "<td>" + items[i].res_time + "시</td>\n";
...
function deleteUid(res_num){
    ...
    if(!confirm(res_num + "번 예매를 취소하시겠습니까?")) return false;
