...
public class userController {
	...
	public void read(@RequestParam("id") int id, Authentication authentication, Model model) throws Exception {
		logger.info("read......" + id);
		if (authentication != null) {
			/*
			 * // 현재 로그인한 사용자의 권한 정보 가져오기 Collection<? extends GrantedAuthority> authorities
			 * = authentication.getAuthorities(); // 권한 정보를 바탕으로 역할(role)을 추출하여 모델에 추가 if
			 * (authorities != null) {
			 * logger.info("----------dhksfy-------------------------"); String role =
			 * getRole(authorities); model.addAttribute("role", role); } else {
			 * model.addAttribute("role", "null"); }
			 */
			UsernamePasswordAuthenticationToken upat = (UsernamePasswordAuthenticationToken) authentication;
			Collection<GrantedAuthority> role = upat.getAuthorities();
	...
	private String getRole(Collection<? extends GrantedAuthority> authorities) {
		for (GrantedAuthority authority : authorities) {
			...
			// 다른 역할에 대한 처리도 추가할 수 있습니다.
		}
		return "User"; // 기본적으로는 User 역할로 설정
	}
